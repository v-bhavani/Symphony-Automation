...
Local_csv_path="/home/ec2-user/ECR_ANUGAL_SCAN_REPORT_local$(date +"%d-%m-%Y").csv"
Final_csv_path="/home/ec2-user/ECR_ANUGAL_SCAN_REPORT_$(date +"%d-%m-%Y").xlsx"
execute_command() {
    local command="$1"
...
aws s3 rm s3://$S3_BUCKET_NAME/$S3_KEY_PREFIX/$anugal_report_id.csv >> "$LOG_FILE" 2>&1
if [ $? -ne 0 ]; then
